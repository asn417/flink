 ...
 * @Description:
 **/
public class ADS_ReceiptBillEntrySum2_OPF implements OutputFormat<ADS_ReceiptBillEntrySum2Vo> {
    private static final Logger logger = LoggerFactory.getLogger(ADS_ReceiptBillEntrySum2_OPF.class);
    ...
    public void open(int i, int i1) throws IOException {
        ...
        if (!admin.tableExists(TableName.valueOf("ads_owner_cloud:ads_receiptBillEntrySum2"))){
            try {
            ...
            createTable("ads_owner_cloud:ads_receiptBillEntrySum2","cf");
        }

        table = conn.getTable(TableName.valueOf("ads_owner_cloud:ads_receiptBillEntrySum2"));
    }

    @Override
    public void writeRecord(ADS_ReceiptBillEntrySum2Vo ads_receiptBillEntrySumVo) throws IOException {
        ...
        String rowkey = RowKeyUtil.generateShortUuid8(MD5Utils.hash(settlementTypeID+moneydefineID))+businessType+tranDate;
