package entity.ods;

import java.io.Serializable;

/**
 * @Author: wangsen
 * @Date: 2020/4/15 14:10
 * @Description:项目维表
 **/
public class ODS_ProjectVo implements Serializable {
    private static final long serialVersionUID = -6999972716390130317L;
    private String iD;
    private String number;
    private String name;
    private String extendName;
    private String pcID;
    private String pcName;
    private String invalidDate;
    private String creator;
    private String createTime;
    private String eCID;
    private String province;
    private String city;
    private String area;
    private String address;
    private String developer;
    private String cFolderID;
    private String orgID;
    private String imageUrl;
    private String agtCode;
    private String departID;
    private String departType;
    private String latitude;
    private String longitude;
    private String tryBegindate;
    private String tryEnddate;
    private String begindate;
    private String enddate;
    private String pdAgelimit;
    private String isDelay;
    private String status;
    private String deleteTime;
    private String isDelete;
    private String isGenerateReceive;
    private String hasGroup;
    private String isOpenMerchant;
    private String carPrefix;
    private String phone;
    private String cloudDeptID;
    private String isRecordTaxRate;
    private String lifeMatching;
    private String transportation;
    private String restaurant;
    private String imageIDs;
    private String projectProperty;
    private String departmentID;
    private String isSingleBuild;
    private String linkPhone;
    private String linkPeople;
    private String areaID;
    private String wxQRCodeID;
    private String easID;
    private String easName;
    private String zone;
    private String sourceID;
    private String sourceSystem;
    private String mchNo;
    private String mchName;
    private String mchUser;
    private String mchPass;
    private String fileIDs;
    private String lanType;
    private String product;
    private String description;

    public String getiD() {
        return this.iD;
    }

    public void setiD(final String iD) {
        this.iD = iD;
    }

    public String getNumber() {
        return this.number;
    }

    public void setNumber(final String number) {
        this.number = number;
    }

    public String getName() {
        return this.name;
    }

    public void setName(final String name) {
        this.name = name;
    }

    public String getExtendName() {
        return this.extendName;
    }

    public void setExtendName(final String extendName) {
        this.extendName = extendName;
    }

    public String getPcID() {
        return this.pcID;
    }

    public void setPcID(final String pcID) {
        this.pcID = pcID;
    }

    public String getPcName() {
        return this.pcName;
    }

    public void setPcName(final String pcName) {
        this.pcName = pcName;
    }

    public String getInvalidDate() {
        return this.invalidDate;
    }

    public void setInvalidDate(final String invalidDate) {
        this.invalidDate = invalidDate;
    }

    public String getCreator() {
        return this.creator;
    }

    public void setCreator(final String creator) {
        this.creator = creator;
    }

    public String getCreateTime() {
        return this.createTime;
    }

    public void setCreateTime(final String createTime) {
        this.createTime = createTime;
    }

    public String geteCID() {
        return this.eCID;
    }

    public void seteCID(final String eCID) {
        this.eCID = eCID;
    }

    public String getProvince() {
        return this.province;
    }

    public void setProvince(final String province) {
        this.province = province;
    }

    public String getCity() {
        return this.city;
    }

    public void setCity(final String city) {
        this.city = city;
    }

    public String getArea() {
        return this.area;
    }

    public void setArea(final String area) {
        this.area = area;
    }

    public String getAddress() {
        return this.address;
    }

    public void setAddress(final String address) {
        this.address = address;
    }

    public String getDeveloper() {
        return this.developer;
    }

    public void setDeveloper(final String developer) {
        this.developer = developer;
    }

    public String getcFolderID() {
        return this.cFolderID;
    }

    public void setcFolderID(final String cFolderID) {
        this.cFolderID = cFolderID;
    }

    public String getOrgID() {
        return this.orgID;
    }

    public void setOrgID(final String orgID) {
        this.orgID = orgID;
    }

    public String getImageUrl() {
        return this.imageUrl;
    }

    public void setImageUrl(final String imageUrl) {
        this.imageUrl = imageUrl;
    }

    public String getAgtCode() {
        return this.agtCode;
    }

    public void setAgtCode(final String agtCode) {
        this.agtCode = agtCode;
    }

    public String getDepartID() {
        return this.departID;
    }

    public void setDepartID(final String departID) {
        this.departID = departID;
    }

    public String getDepartType() {
        return this.departType;
    }

    public void setDepartType(final String departType) {
        this.departType = departType;
    }

    public String getLatitude() {
        return this.latitude;
    }

    public void setLatitude(final String latitude) {
        this.latitude = latitude;
    }

    public String getLongitude() {
        return this.longitude;
    }

    public void setLongitude(final String longitude) {
        this.longitude = longitude;
    }

    public String getTryBegindate() {
        return this.tryBegindate;
    }

    public void setTryBegindate(final String tryBegindate) {
        this.tryBegindate = tryBegindate;
    }

    public String getTryEnddate() {
        return this.tryEnddate;
    }

    public void setTryEnddate(final String tryEnddate) {
        this.tryEnddate = tryEnddate;
    }

    public String getBegindate() {
        return this.begindate;
    }

    public void setBegindate(final String begindate) {
        this.begindate = begindate;
    }

    public String getEnddate() {
        return this.enddate;
    }

    public void setEnddate(final String enddate) {
        this.enddate = enddate;
    }

    public String getPdAgelimit() {
        return this.pdAgelimit;
    }

    public void setPdAgelimit(final String pdAgelimit) {
        this.pdAgelimit = pdAgelimit;
    }

    public String getIsDelay() {
        return this.isDelay;
    }

    public void setIsDelay(final String isDelay) {
        this.isDelay = isDelay;
    }

    public String getStatus() {
        return this.status;
    }

    public void setStatus(final String status) {
        this.status = status;
    }

    public String getDeleteTime() {
        return this.deleteTime;
    }

    public void setDeleteTime(final String deleteTime) {
        this.deleteTime = deleteTime;
    }

    public String getIsDelete() {
        return this.isDelete;
    }

    public void setIsDelete(final String isDelete) {
        this.isDelete = isDelete;
    }

    public String getIsGenerateReceive() {
        return this.isGenerateReceive;
    }

    public void setIsGenerateReceive(final String isGenerateReceive) {
        this.isGenerateReceive = isGenerateReceive;
    }

    public String getHasGroup() {
        return this.hasGroup;
    }

    public void setHasGroup(final String hasGroup) {
        this.hasGroup = hasGroup;
    }

    public String getIsOpenMerchant() {
        return this.isOpenMerchant;
    }

    public void setIsOpenMerchant(final String isOpenMerchant) {
        this.isOpenMerchant = isOpenMerchant;
    }

    public String getCarPrefix() {
        return this.carPrefix;
    }

    public void setCarPrefix(final String carPrefix) {
        this.carPrefix = carPrefix;
    }

    public String getPhone() {
        return this.phone;
    }

    public void setPhone(final String phone) {
        this.phone = phone;
    }

    public String getCloudDeptID() {
        return this.cloudDeptID;
    }

    public void setCloudDeptID(final String cloudDeptID) {
        this.cloudDeptID = cloudDeptID;
    }

    public String getIsRecordTaxRate() {
        return this.isRecordTaxRate;
    }

    public void setIsRecordTaxRate(final String isRecordTaxRate) {
        this.isRecordTaxRate = isRecordTaxRate;
    }

    public String getLifeMatching() {
        return this.lifeMatching;
    }

    public void setLifeMatching(final String lifeMatching) {
        this.lifeMatching = lifeMatching;
    }

    public String getTransportation() {
        return this.transportation;
    }

    public void setTransportation(final String transportation) {
        this.transportation = transportation;
    }

    public String getRestaurant() {
        return this.restaurant;
    }

    public void setRestaurant(final String restaurant) {
        this.restaurant = restaurant;
    }

    public String getImageIDs() {
        return this.imageIDs;
    }

    public void setImageIDs(final String imageIDs) {
        this.imageIDs = imageIDs;
    }

    public String getProjectProperty() {
        return this.projectProperty;
    }

    public void setProjectProperty(final String projectProperty) {
        this.projectProperty = projectProperty;
    }

    public String getDepartmentID() {
        return this.departmentID;
    }

    public void setDepartmentID(final String departmentID) {
        this.departmentID = departmentID;
    }

    public String getIsSingleBuild() {
        return this.isSingleBuild;
    }

    public void setIsSingleBuild(final String isSingleBuild) {
        this.isSingleBuild = isSingleBuild;
    }

    public String getLinkPhone() {
        return this.linkPhone;
    }

    public void setLinkPhone(final String linkPhone) {
        this.linkPhone = linkPhone;
    }

    public String getLinkPeople() {
        return this.linkPeople;
    }

    public void setLinkPeople(final String linkPeople) {
        this.linkPeople = linkPeople;
    }

    public String getAreaID() {
        return this.areaID;
    }

    public void setAreaID(final String areaID) {
        this.areaID = areaID;
    }

    public String getWxQRCodeID() {
        return this.wxQRCodeID;
    }

    public void setWxQRCodeID(final String wxQRCodeID) {
        this.wxQRCodeID = wxQRCodeID;
    }

    public String getEasID() {
        return this.easID;
    }

    public void setEasID(final String easID) {
        this.easID = easID;
    }

    public String getEasName() {
        return this.easName;
    }

    public void setEasName(final String easName) {
        this.easName = easName;
    }

    public String getZone() {
        return this.zone;
    }

    public void setZone(final String zone) {
        this.zone = zone;
    }

    public String getSourceID() {
        return this.sourceID;
    }

    public void setSourceID(final String sourceID) {
        this.sourceID = sourceID;
    }

    public String getSourceSystem() {
        return this.sourceSystem;
    }

    public void setSourceSystem(final String sourceSystem) {
        this.sourceSystem = sourceSystem;
    }

    public String getMchNo() {
        return this.mchNo;
    }

    public void setMchNo(final String mchNo) {
        this.mchNo = mchNo;
    }

    public String getMchName() {
        return this.mchName;
    }

    public void setMchName(final String mchName) {
        this.mchName = mchName;
    }

    public String getMchUser() {
        return this.mchUser;
    }

    public void setMchUser(final String mchUser) {
        this.mchUser = mchUser;
    }

    public String getMchPass() {
        return this.mchPass;
    }

    public void setMchPass(final String mchPass) {
        this.mchPass = mchPass;
    }

    public String getFileIDs() {
        return this.fileIDs;
    }

    public void setFileIDs(final String fileIDs) {
        this.fileIDs = fileIDs;
    }

    public String getLanType() {
        return this.lanType;
    }

    public void setLanType(final String lanType) {
        this.lanType = lanType;
    }

    public String getProduct() {
        return this.product;
    }

    public void setProduct(final String product) {
        this.product = product;
    }

    public String getDescription() {
        return this.description;
    }

    public void setDescription(final String description) {
        this.description = description;
    }
}
